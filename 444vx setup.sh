...
update_system() {
    ...
    case "$PKG_FAMILY" in
        debian)
            ...
            apt-get update -qq
            apt-get upgrade -y -q
            # software-properties-common nie istnieje na wszystkich dystrybucjach
            # (np. MX Linux) — instalujemy osobno z || true żeby nie blokować
            apt-get install -y -q \
                curl wget apt-transport-https gnupg lsb-release \
                flatpak pciutils \
                || error "Nie udało się zainstalować podstawowych zależności (Debian)"
            apt-get install -y -q software-properties-common 2>/dev/null || \
                warn "software-properties-common niedostępne na tej dystrybucji — PPA nie będą działać (użyty zostanie Flatpak)"
            ;;
        arch)
